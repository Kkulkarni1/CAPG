...
int match_indels(merge_hash *mh, sam **sds, ref_info *rfi)
{
	...
	for (merge_hash *me = mh; me != NULL; me = me->hh.next) {
		...
		unsigned int last_rf_idx, last_rd_idx, last_other_rf_idx;
		...
		for (unsigned int i = 0; i < se->cig->n_ashes; ++i) {
			...
			for (unsigned int j = 0; j < se->cig->ashes[i].len; ++j) {
				...
				/* aligns to target region in both subgenomes */
				if (rf_idx >= 0 && other_rf_idx >= 0) {
					int alt_rf_idx = rfi->map_B_to_A[other_rf_idx];	/* sgA index implied by B alignment */
					int alt_other_rf_idx = rfi->map_A_to_B[rf_idx];	/* sgB index implied by A alignment */

					/* homoeologous alignment */
					if (rf_idx == alt_rf_idx) {
						...
						if (in_discrepancy) {
							...
							if (lla > llb) {
								match_indel(me, rfi, last_rd_idx, rd_idx, 0);
							} else if (llb > lla) {
								match_indel(me, rfi, last_rd_idx, rd_idx, 1);
							} else {
...
int match_indel(merge_hash *me, sam **sds, ref_info *rfi, unsigned int start_rd_idx, unsigned int end_rd_idx, unsigned int sg_idx)
{
	...
	/* accepting sgB alignment */
	if (sg_idx) {
		/* sgA alignment */
		sam_entry *se = &sds[!sg_idx]->se[me->indices[!sg_idx][0]];
		unsigned char n_region = 0;

		/* B reference index aligned to homoeologous site right before discrepant region */
		unsigned int other_rf_idx = rfi->read_to_ref[sg_idx][start_rd_idx];
		unsigned int last_other_rf_idx = other_rf_idx;
		int prev_match = ALIGN_MATCH;
		...
		for (unsigned int j = 0; j < se->cig->n_ashes; ++j) {
			...
			for (unsigned int k = 0; k < se->cig->ashes[j].len; ++k) {
				...
				/* continuing in region */
					...
					other_rf_idx = rfi->read_to_ref[1][rd_idx + k];		/* other reference index aligned to this read nucleotide */
					unsigned int alt_rf_idx = other_rf_idx >= 0
					...
					if (other_rf_idx > last_other_rf_idx + 1) {
						++n_new_ashes;
				...
				/* entering region: must enter homoeologous INDEL */
					...
					other_rf_idx = rfi->read_to_ref[1][rd_idx + k];
					unsigned int alt_rf_idx = other_rf_idx >= 0
		...
		ash *ashes = se[0]->cig->ashes;
		if (n_new_ashes != n_old_ashes) {
			ash *ashes = malloc((se[0]->cig->n_ashes + n_new_ashes - n_old_ashes) * sizeof(*ashes));

			if (!ashes)
				return(mmessage(ERROR_MSG, MEMORY_ALLOCATION, "ashes"));
		}

		/* build the new cigar for sgA alignment based on sgB alignment */
		rd_idx = 0;	/* this is read nucleotide with homoeologous alignment right before the discrepant region */
		last_other_rf_idx = rfi->read_to_ref[1][start_rd_idx];
		prev_ash = ALIGN_MATCH;
		n_ash = 0;
		for (unsigned int j = 0; j < se->cig->n_ashes; ++j) {
			if (se->cig->ashes[j].type == CIGAR_SOFT_CLIP) {
			...
			/* insertion or match */
			for (unsigned int k = 0; k < se[0]->cig->ashes[j].len; ++k) {

				/* stepping out of region */
				if (in_region && rd_idx[0] + k >= end_rd_idx) {
					/* TODO resolve it */
					break;

				/* continuing in region */
				} else if (in_region) {
					other_rf_idx = rfi->read_to_ref[1][rd_idx[0] + k];
					unsigned int alt_rf_idx = rfi->map_B_to_A[other_rf_idx];	/* A ref index or ALIGN_INSERTION or ALIGN_DELETION */
				 ...
				 * match
				 */
				} else if (!in_region && rd_idx[0] + k > start_rd_idx) {
					other_rf_idx = rfi->read_to_ref[1][rd_idx[0] + k];
					unsigned int alt_rf_idx = rfi->map_B_to_A[other_rf_idx];	/* ALIGN_INSERTION or ALIGN_DELETION */
					...
					if (other_rf_idx > last_other_rf_idx + 1) {
						...
						/* insertion in sgB wrt sgA is indicates deletion in R wrt sgA */
			...
			rd_idx[0] += se[0]->cig->ashes[j].len;
			rf_idx[0] += se[0]->cig->ashes[j].len;
			if (rd_idx[0] >= end_rd_idx)
				break;
		}
		if (n_new_ashes > n_old_ashes) {
			free(se[0]->cig->ashes);
			se[0]->cig->n_ashes += n_new_ashes - n_old_ashes;
		} else if (n_old_ashes > n_new_ashes) {
			free(se[0]->cig->ashes);
			se[0]->cig->n_ashes -= n_old_ashes - n_new_ashes;
		}
		se[0]->cig->ashes = ashes;
						...
						/* to sgA */
						rn = ref[(size_t) rf_idx + rfi->start_A - rfi->alignment_start[0]];
						...
						/* to sgB */
						rn = ref[(size_t) alt_other_rf_idx + rfi->start_B - rfi->alignment_start[1]];
						lla += sub_prob_given_q_with_encoding(rn,
							get_nuc(se->read, XY_ENCODING, rd_index + j),
							IUPAC_ENCODING, XY_ENCODING,
							get_qual(se->qual, rd_index + j), 1, (void *) mls);

						/* log likelihood of alternate alignment */
						/* to sgA */
						rn = ref[(size_t) alt_rf_idx + rfi->start_A - rfi->alignment_start[0]];
						...
						llb += sub_prob_given_q_with_encoding(rn,
							...
							get_qual(se->qual, rd_index + j), 1, (void *) mls);
						/* to sgB */
						rn = ref[(size_t) other_rf_idx + rfi->start_B - rfi->alignment_start[1]];
						/* log likelihood of this alternative */
						llb += sub_prob_given_q_with_encoding(rn,
							get_nuc(se->read, XY_ENCODING, rd_index + j),
							IUPAC_ENCODING, XY_ENCODING,
			...
			rd_idx += se->cig->ashes[i].len;
			if (n_bank && last_banked_rf_idx +  rfi->start_A < rf_idx) {
				rf_idx += se->cig->ashes[i].len;
				if (se->cig->ashes[i].len == n_bank) {
					--se->cig->n_ashes;
					for (unsigned int k = i; k < se->cig->n_ashes; ++k) {
						se->cig->ashes[k].len = se->cig->ashes[k+1].len;
						se->cig->ashes[k].type = se->cig->ashes[k+1].type;
					}
					se->cig->ashes = realloc(se->cig->ashes, se->cig->n_ashes * sizeof(*se->cig->ashes));
				} else {
					se->cig->ashes[i].len -= n_bank;
				}
				se->cig->ashes[i-1].len += n_bank;
				n_bank = 0;
				last_banked_rf_idx = -1;
			} else if (n_bank) {
				rf_idx += se->cig->ashes[i].len;
				n_bank = 0;
				last_banked_rf_idx = -1;
				...
				for (unsigned int k = 0; k < se->cig->ashes[i].len; ++k) {
					unsigned int lrf_idx = rfi->read_to_ref[0][rd_idx + k];

					/* aligned to target in this subgenome */
					if (lrf_idx != -1) {
						unsigned int other_rf_idx = rfi->read_to_ref[1][rd_idx + k];

						/* aligned to target in other subgenome */
						if (other_rf_idx != -1) {
							unsigned int arf_idx = rfi->map_B_to_A[other_rf_idx];

							/* but not homoeologous alignment */
							if (arf_idx > lrf_idx) {
								++n_insertion;
								last_insertion_rf_idx = lrf_idx;
							/* homoeologous alignment */
							} else if (arf_idx == lrf_idx && n_insertion) {
								n_insertion = 0;
								last_insertion_rf_idx = 0;
							}
						}
					}
				}
