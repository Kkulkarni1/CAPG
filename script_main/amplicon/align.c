...
/**
 * Align IUPAC-encoded sequence to IUPAC-encoded profile.  A profile is a
 * set of already aligned sequences, usually 2, encoded using ambiguous
 * nucleotides, like R = A | G.
 *
 * @param seq		the IUPAC-encoded sequence
 * @param prof		the IUPAC-encoded profile
 * @param len1		length of the sequence
 * @param len2		length of the profile
 * @param match		match score (e.g., 1)
 * @param mismatch	mismatch score (e.g., -2)
 * @param gap_p		gap penalty (linear)
 * @param band		size of banded alignment (-1 for not banded)
 * @param semi		semi-global alignment
 * @param err		pointer to error status
 * @param alen		pointer to length of alignment
 * @param ascr		pointer to alignment score
 * @return		alignment
 */
unsigned char **nw_seq_profile_align(char_t const * const seq,
	char_t const * const prof, int len1, int len2,
	int match, int mismatch, double gap_p,
	int band, int semi, int *err,
	unsigned int *alen, double *ascr)
{
	static size_t nnw = 0;
	int i, j;
	int l, r;
	...
	*err = NO_ERROR;
	...
	for (i = 1; i <= len1; i++) {
		...
		for (j = l; j <= r; j++) {
			...
			// Score for the diagonal move; gaps in profile do not cost.
			diag = d[(i-1)*ncol + j-1]
				+ popcnt[seq[i-1] & prof[j-1]] * match
				+ (popcnt[prof[j-1]] - popcnt[seq[i-1] & prof[j-1]]) * mismatch;
